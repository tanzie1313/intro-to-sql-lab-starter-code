...
-- Write SQL query here
select countries.code, countries.name, countrylanguages.countrycode, countrylanguages.language, countrylanguages.percentage
from countries
inner join countrylanguages on countries.code = countrylanguages.countrycode where countrylanguages.percentage=100 and countrylanguages.language='Italian';
...
-- select * from cities where name like '%Serra%';
 SELECT cities.name, cities.countrycode, countries.region
 FROM cities
 INNER JOIN countries ON cities.countrycode = countries.code where countries.region='South America' and countries.code='BRA' and cities.name LIKE 'Serra%';
